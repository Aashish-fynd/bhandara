...
export const getDirectories = (source) => readdirSync(__dirname(source), { withFileTypes: true })
    .filter((dirent) => dirent.isDirectory())
    .map((dirent) => dirent.name);
/** List route files within a directory. */
export const getRouteFiles = (source) => readdirSync(__dirname(source), { withFileTypes: true })
    .filter((dirent) => !dirent.isDirectory() && dirent.name.includes("route"))
    .map((dirent) => dirent.name);
/** Return all file names within a directory. */
export const getDirectoryFiles = (source) => readdirSync(__dirname(source), { withFileTypes: true })
    .filter((dirent) => !dirent.isDirectory())
    .map((dirent) => dirent.name);
