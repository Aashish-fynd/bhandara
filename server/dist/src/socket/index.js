...
import { EventService, getSafeUser, MediaService, MessageService, ReactionService, ThreadService, getExplorePage, setExplorePage, deleteExplorePage, buildExploreSections, } from "@features";
...
const createJoinRoom = (socket, room) => {
    socket.join(room);
    rooms.add(room);
};
...
export function initializeSocket(server) {
    ...
    platformNamespace.use((socket, next) => requestContextMiddleware(socket.request, null, next));
    platformNamespace.use(socketUserParser);
    platformNamespace.on(PLATFORM_SOCKET_EVENTS.CONNECT, async (socket) => {
        ...
        socket.on(PLATFORM_SOCKET_EVENTS.REACTION_UPDATED, async (request, cb) => {
            try {
                ...
                if (!content)
                    throw new NotFoundError(`Content not found with provided id:${contentId}`);
                // Check if the content is in a locked thread
                if (contentPath === EAllowedReactionTables.Message) {
                ...
                if (!previousReaction)
                    throw new NotFoundError(`Reaction not found ${reaction} for user`);
        ...
        socket.on(PLATFORM_SOCKET_EVENTS.REACTION_DELETED, async (request, cb) => {
            try {
                ...
                if (!previousReaction)
                    throw new NotFoundError(`Reaction not found ${reaction} for user`);
                const deletedReaction = await reactionService.delete(previousReaction.id, true);
        ...
        socket.on(PLATFORM_SOCKET_EVENTS.EXPLORE, async (request, cb) => {
            ...
            socket.exploreAbort = abortController;
            try {
                let page = ((await getExplorePage(socketUserId)) || 1);
                while (!abortController.signal.aborted) {
                    const { items, pagination } = await eventService.getAll({ status: [EEventStatus.Ongoing, EEventStatus.Upcoming] }, { limit: 10, page });
                    let radius = 100;
                    let nearby = items;
        ...
        socket.on(PLATFORM_SOCKET_EVENTS.DISCONNECT, async () => {
            const ctrl = socket.exploreAbort;
            if (ctrl)
                ctrl.abort();
