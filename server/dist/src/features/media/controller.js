...
export const getSignedUploadUrl = async (req, res) => {
    let { path, bucket, mimeType, provider, parentPath, format, ...rest } = req.body;
...
export const getPublicSignedUploadUrl = async (req, res) => {
...
export const createMediaData = async (req, res) => {
    const { file, path, bucket, mimeType, provider, format, ...rest } = req.body;
    const uploadPath = `${req.user.id}/${path}`;
...
export const getMediaPublicUrl = async (req, res) => {
    ...
    if (isEmpty(signedUrl))
        throw new NotFoundError("Media not found at path");
...
export const getMediaPublicUrls = async (req, res) => {
    const { ids } = req.query;
...
export const onUploadComplete = async (req, res) => {
    ...
    if (!rid)
        throw new BadRequestError(`Missing context id`);
