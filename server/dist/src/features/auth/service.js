...
class Auth {
    ...
    performOAuth = async (provider, redirectTo) => {
    ...
    signUpNewUser = async (email, password, redirectTo) => {
    ...
    createPlatformUser = async ({ sessionData, req, existingUser, }) => {
        ...
        if (!existingUser) {
            ...
            const newUserData = {
                ...
                isVerified: Object.values(EAuthProvider).includes(authProvider),
                profilePic,
            ...
            existingUser = newUser?.[0] ?? newUser;
        }
