...
class SearchService {
     ...
     */
    async searchEvents(query, filters, limit, offset) {
        const whereClause = {
            [Op.or]: [
                {
                    name: {
        ...
        const results = rows.map((event) => {
            ...
            const previewImage = event.media?.find((m) => m.type === 'image')?.publicUrl;
            return {
                id: event.id,
                type: 'event',
                title: event.name,
     ...
     * Search users
     */
    async searchUsers(query, filters, limit, offset) {
        const whereClause = {
            [Op.or]: [
                {
                    username: {
     ...
     * Search tags
     */
    async searchTags(query, filters, limit, offset) {
        const whereClause = {
            [Op.or]: [
                {
                    name: {
    ...
    calculateEventRelevance(event, query) {
        ...
        // Tag match
        if (event.tags?.some((tag) => tag.toLowerCase().includes(queryLower))) {
            score += 4;
        }
        // Recency bonus
     ...
     */
    calculateTagRelevance(tag, query) {
        let score = 0;
        const queryLower = query.toLowerCase();
        // Exact name match
