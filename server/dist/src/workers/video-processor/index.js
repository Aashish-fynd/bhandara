...
const convertToWebP = async (inputPath, outputPath, size, fps = 10) => {
...
export const processor = async (job) => {
    const { mediaId, eventId } = job.data;
    try {
        const media = await mediaService.getById(mediaId);
        if (!media)
            return;
        const { signedUrl } = await mediaService.getPublicUrl(media.url, media.storage.bucket, media.storage.provider, { download: true });
        ...
        const uploaded = await Promise.all(Object.entries(thumbBuffers).map(([suffix, buffer]) => mediaService.uploadFile({
