import { IMessage, IPaginationParams } from "@/definitions/types/global";
...
class MessageService extends Base<IMessage> {
  ...
  async getAll(args: BaseQueryArgs<IMessage> = {}, pagination: Partial<IPaginationParams> = {}) {
    args.query?.push({
      value: null,
    ...
    const childMessagesPromises = (parentLevelMessages?.items || [])?.map((m) => this.getChildren(m.id, { limit: 1 }));
    ...
    const messagesWithChildren = parentLevelMessages?.items?.map((parent, index) => ({
      ...parent,
  ...
  async getChildren(parentId: string, pagination: Partial<IPaginationParams> = {}) {
    return super.getAll({
      query: [{ value: parentId, operator: EQueryOperator.Eq, column: "parentId" }],
      ...pagination
    });
  }

  async create<U extends Partial<Omit<IMessage, "id" | "updatedAt">>>(data: U, useTransaction?: boolean) {
    return validateMessageCreate(data, (data) => super.create(data, useTransaction));
  }
